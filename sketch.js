...
function setup() {//configuración del juego
  ...
  piso = createSprite(width / 2, height - 70, width, 10);
  piso.shapeColor = "brown";
  piso.addImage(pisoImagen);
  piso.velocityX = -3;

  pisoInvisible = createSprite(width / 2, height - 60, width, 12);
...
function draw() {//Dibujar
  background("white");
  drawSprites();
  text("score " + puntos, width - 100, 50)
  ...
  if (gameState == PLAY) {
    ...
    crearNubes();
    crearObstaculos();
    if (touches.lenght > 0 || keyDown("space") && Trex.y > 155) {
      Trex.velocityY = -12;
      touches = [];
    }
...
function crearNubes() {
  ...
  if (frameCount % 40 === 0) {
    ...
    nube = createSprite(width + 20, height - 250 - altura, 30, 10);
...
function crearObstaculos() {
  if (frameCount % 50 === 0) {
    var cactus = createSprite(width, height - 90, 20, 70);
    cactus.velocityX = -6;
